...
class ProductController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $products = Product::with(['productOutlets', 'category', 'brand', 'unit'])->orderByDesc('created_at');

            return DataTables::of($products)
                ->addColumn('outlets', fn($row) => $row->productOutlets->count() . ' outlet')
                ->addColumn('category', fn($row) => $row->category->name ?? '-')
                ->addColumn('brand', fn($row) => $row->brand->name ?? '-')
                ->addColumn('unit', fn($row) => $row->unit->name ?? '-')
                ->addColumn('stock', fn($row) => $row->getTotalStock() ?? 0)
    ...
    public function store(Request $request)
    {
        ...
        // Create product outlet record with initial stock
        ProductOutlet::create([
            'product_id' => $product->id,
            'outlet_id' => $request->outlet_id,
            'stock' => $request->stock,
            'reorder_point' => $request->min_stock ?? 0,
        ]);
    ...
    public function show(string $id)
    {
        ...
        $product = Product::with(['productOutlets.outlet', 'category', 'brand', 'unit'])->findOrFail($productId);
